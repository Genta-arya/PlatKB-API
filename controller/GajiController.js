...
export const getGajiTeknisi = async (req, res) => {
  try {
    // Ambil semua data gaji tanpa include relasi
    const allGaji = await prisma.gajiMekanik.findMany();
    ...
    // Ambil data mekanik satu per satu (biar aman kalau relasi rusak)
    const gajiDenganMekanik = await Promise.all(
      allGaji.map(async (gaji) => {
        const mekanik = await prisma.mekanik.findUnique({
          where: { id: gaji.mekanikId },
        });

        // Jika tidak ditemukan, abaikan data ini
    ...
    // Buang gaji yang relasi mekaniknya tidak ditemukan
    const validGaji = gajiDenganMekanik.filter(Boolean);

    if (validGaji.length === 0) {
      return res
        .status(200)
        .json({ message: "Semua data gaji rusak (mekanik tidak ditemukan)", data: [] });
    }
    ...
    const filteredGaji = validGaji.filter((gaji) => {
      const gajiTanggalAkhir = new Date(gaji.tanggal_akhir);
      return dateObj <= gajiTanggalAkhir;
    });
